...
class PlantaList(generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['planta_list'] = Planta.objects \
            ...
            .order_by('nombre')
        logger.error( context['planta_list'] )
        return context
...
class PlantaDepreciacion(generic.ListView):
    ...
    def toCSV(request):
        ...
        response['Content-Disposition'] = 'attachment; filename="deprec_plantas.csv"'

        writer = csv.writer(response, delimiter=';')
        writer.writerow(['Planta', 'Periodo', 'Depreciacion', 'Valor Contable'])
        for fila in ActivoDepreciacion.objects \
            ...
            writer.writerow([ fila['planta__nombre'], fila['periodo'], fila['depreciacion'], fila['contable'] ])
        return response
...
class PlantaDetail(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['activo_list'] = Activo.objects.filter(planta=super().get_object().id)
...
class PlantaDepreciacionMensual(generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        logger.error( 'PlantaDepreciacionMensual get_context_data' )
        logger.error( super().get_object() )

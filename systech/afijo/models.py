...
#          verbose_name_plural = "depreciación anual"
...
class Activo(models.Model):
    ...
    tipoDepreciacion = models.ForeignKey(TipoDepreciacion, default=1, on_delete=models.CASCADE)
    nombre = models.CharField('Nombre',max_length=80)
    fecha_ingreso = models.DateField('Fecha Ingreso')
    fecha_inicio = models.DateField('Fecha Inicio Depreciación', null=True, blank=True)
    duracion_maxima = models.IntegerField('Duración Depreciación [años]',default=5)
    fecha_termino = models.DateField('Fecha Término Depreciación', null=True, blank=True)
    ...
    tipoActivo = models.CharField(max_length=1, choices=TipoActivo, default='M')
    ...
    def calculaDepreciacion(self):
        ...
        periodoIni = dateToPeriodo(self.fecha_inicio if self.fecha_inicio > self.planta.fecha_depreciacion else self.planta.fecha_depreciacion)
        ...
        if periodoBaja==None: periodoBaja =periodoFin
    ...
    def pre_save( sender, instance, **kwargs ):
        ...
        if not instance.fecha_inicio is None and instance.fecha_termino is None :
            instance.fecha_termino = instance.fecha_inicio + relativedelta(years=instance.duracion_maxima)
...
class Movimiento(models.Model):
    ...
    planta_origen = models.ForeignKey(Planta, related_name='plantaOrigen', on_delete=models.CASCADE)
    planta_destino = models.ForeignKey(Planta, related_name='plantaDestino', on_delete=models.CASCADE)
    ts_movim = models.DateTimeField('Fecha Registro', default=timezone.now, editable=False)
    ...
    def post_save( sender, instance, created, **kwargs ):
        ...
        try: 
            actDep = ActivoDepreciacion.objects.get(activo=instance.activo,periodo=periodoBaja)
        except:
            actDep = None
        ...
        if actDep!=None:
            ...
            activoNuevo.duracion_maxima = activoNuevo.duracion_maxima - diff_meses(dateToPeriodo(activoNuevo.fecha_inicio), periodoBaja)
            activoNuevo.save()
        else:
            pass
